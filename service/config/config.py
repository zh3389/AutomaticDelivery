...
def init_db(update=False):
    ...
    db.session.add(Payment('Stripe支付宝', '支付宝', "{'key':'sk_xxx','currency':'cny'}", 'stripe.com综合收款系统(已完成逻辑，但未实测,缺少反馈)', False))
    db.session.add(Payment('Stripe微信', '微信支付', "{'key':'sk_xxx','currency':'usd'}", 'stripe.com综合收款系统(aud, cad, eur, gbp, hkd, jpy, sgd, usd)', False))

    # 商品分类
    db.session.add(ProdCag('免费专区', '虚拟账号类商品', '1'))
    # 商品设置
    db.session.add(ProdInfo('免费专区', '普通商品演示', '商品简述信息演示XXXX', 'images/null.png', '100', '示例：卡密格式：账号------密码-----', 9.99, None, True, 0, '请填写邮箱', True))
    db.session.add(ProdInfo('免费专区', '批发商品演示', '商品简述信息演示XXXX', 'images/null.png', '100', '示例：卡密格式：账号------密码-----', 9.99, '9,100#9.9,8.82,7.7', True, 0, '请填写邮箱', True))
    # 卡密设置
    ...
    db.session.add(Config('contact_us', '<p>如购买出现问题, 联系客服QQ 24小时完成处理.</p>', '首页-联系我们', True))
    db.session.add(Config('web_footer', '<a style="color: #fafafa;" href="https://www.beianx.cn/">蜀ICP备2022025214号-1</a>', '可填写备案信息', True))
    db.session.add(Config('top_notice', '购买如出现问题: 联系客服QQ', '首页公告', True))
    db.session.add(Config('toast_notice', '正常运营 欢迎~', '首页滑动消息设置', True))
    ...
    db.session.add(Config('kamiFaka', 'www.baidu.com', 'Github项目地址，用于手动检测新版', False))
    db.session.add(Config('kamiFaka_v', '6.88', 'Github项目地址，用于手动检测新版', False))
